...
fun AutomationScreen(onOpenSettings: () -> Unit) {
    ...
    LazyColumn(
            ...
            Card(modifier = Modifier.fillMaxWidth(), shape = RoundedCornerShape(24.dp)) {
                Column(modifier = Modifier.padding(20.dp)) {
                    ...
                    OutlinedTextField(
                        ...
                        placeholder = { Text("Example: prepare my study summary") },
                        shape = RoundedCornerShape(18.dp)
                    )
                    Spacer(Modifier.height(10.dp))
                    OutlinedTextField(
                        value = delayMinutes,
                        onValueChange = { if (it.all(Char::isDigit)) delayMinutes = it },
                        modifier = Modifier.fillMaxWidth(),
        ...
        item {
            Text("Scheduled history", style = MaterialTheme.typography.titleLarge, fontWeight = FontWeight.Bold)
        }
        items(infos, key = { it.id }) { info ->
            ListItem(
                ...
                leadingContent = { Icon(if (info.state == WorkInfo.State.SUCCEEDED) Icons.Default.CheckCircle else Icons.Default.Schedule, contentDescription = null) },
                trailingContent = {
                    IconButton(onClick = { workManager.cancelWorkById(info.id) }) {
                        Icon(Icons.Default.DeleteOutline, contentDescription = "Cancel")
                    }
                }
            )
        }
